...
if dkms status | grep -q "bbrx/"; then
	for module_ver in $(dkms status | grep "bbrx/" | awk -F, '{print $1}' | awk -F/ '{print $2}' | sort -u); do
		echo "Removing existing bbrx module version: $module_ver"
		dkms remove -m bbrx -v "$module_ver" --all
	done
fi
